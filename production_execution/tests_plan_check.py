...
class OilProductionConsumptionStockTests(PlanCheckBase):
    """Oil counts only what is already at the line's consumption warehouse.

    Material still in a godown or a tank has to be fetched before the line can
    run on it, so for Oil a plan leaning on it is a shortage — raw and packing
    alike — and needs a written reason to save.
    """
    ...
    def test_packing_in_a_godown_only_is_short(self):
        result = self.oil(
            [bom_row('PM001', 'Caps', 20)],
        ...
        # The godown figure still travels, so the screen can say where it is.
        self.assertEqual(row['sap_on_hand'], 50000)
        self.assertTrue(result['blocking']['requires_remark'])
    ...
    def test_raw_on_the_register_but_not_at_pc_is_short(self):
        RawMaterialStock.objects.create(
            company=self.company, warehouse_code='BH-LO', item_code='RM0000002',
            item_name='Canola oil', uom='LTR', qty=Decimal('32000'),
    ...
    def test_the_bills_own_consumption_warehouse_is_the_one_counted(self):
        row = self.oil(
            [bom_row('PM001', 'Caps', 20, issue_warehouse='BH-PP')],
            [stock_row('PM001', 'BH-PC', 5000), stock_row('PM001', 'BH-PP', 2000)],
        )['materials']['rows'][0]
        self.assertEqual(row['on_hand'], 2000)
        self.assertEqual(row['searched_warehouses'], ['BH-PP'])
