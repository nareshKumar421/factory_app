...
class PermanentLabourStrength(Stamped):
    ...
    One number per company, because that is how the factory speaks about it:
    "we have eighty-five permanent labour". It is a *strength*, not a roster —
    the directory holds named employees, but the shop floor's own workers are
    not all entered there, and waiting until they are would leave the daily
    count with nothing to measure itself against.

    Kept as a row somebody edits rather than derived from a count, so it stays
    true the day three people join and their records are typed in a week later.
    """
    ...
    def __str__(self):
        return f"{self.company.code}: {self.headcount} permanent"
...
class PermanentLabourPresence(Stamped):
    ...
    def __str__(self):
        return f"{self.company_id} {self.work_date} {self.shift}: {self.present_count}/{self.strength}"
